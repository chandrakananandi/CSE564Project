...
\label{sec:policies}
We define five types of security policies that should be satisfied by a home automation system. Our policies focus on the behaviour of the controllers and their interaction with the other devices in a smart home. 
\begin{itemize}
...
\item Temporal policy (\S\ref{subsec:time}): This policy ensures that the events happening in a smart home respect certain timing constraints.

\item New device policy (\S\ref{subsec:newdevice}): This policy states that when a new device is added to a house, it should not cause the home to violate any of the other policies assuming that they were satisfied before the integration of the new device.
\end{itemize}
